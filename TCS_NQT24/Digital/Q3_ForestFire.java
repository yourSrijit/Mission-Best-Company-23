...
public class Q3_ForestFire {
    public static  void calTime(int row,int col,int curTime,int [][]time,char [][]forest){
        //Boundary Check
        ...
        //is Already burned check
        if(time[row][col]!=0 && time[row][col] <=curTime){
            return;
        }
        //Calculation
        time[row][col]=curTime;
        calTime(row-1,col,curTime+1,time,forest);
        calTime(row+1,col,curTime+1,time,forest);
        calTime(row,col-1,curTime+1,time,forest);
        calTime(row,col+1,curTime+1,time,forest);

        calTime(row-1,col+1,curTime+1,time,forest);
        calTime(row+1,col+1,curTime+1,time,forest);
        calTime(row+1,col-1,curTime+1,time,forest);
        calTime(row-1,col-1,curTime+1,time,forest);
    ...
    public static void main(String[] args) {
        ...
        calTime(r-1,c-1,1,time,forest);
        int maxTime=-1;
        for(int i=0;i<row;i++){
            for(int j=0;j<col;j++){
//                System.out.print(time[i][j]+" ");  no need to do this
               if(time[i][j]>maxTime){
                   maxTime=time[i][j];
               }
            }
